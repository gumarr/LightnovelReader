...
| Đọc lại audio + timing (P3.1) | Đường **ghi** (hàng đợi) và đường **đọc** (`reader:getSegmentAudio`) gặp nhau lần đầu. Unit test luôn giả một trong hai đầu nên không bao giờ chứng minh được chúng trỏ cùng một chỗ. Kiểm: bytes qua handler khớp file trên đĩa **từng byte**; `timingSource` là `phoneme` chứ không âm thầm rơi về ước lượng; `charStart`/`charEnd` cắt lại **đúng chữ** trong text gốc (thứ subtitle pane dùng để tô); xoá `.json` thì rơi về `estimate` chứ không trả mảng rỗng; xoá `.ogg` thì `NOT_FOUND` để player xếp lại hàng đợi |
