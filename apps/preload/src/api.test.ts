...
const invokedChannels = async (): Promise<Set<string>> => {
  ...
  const calls: Array<() => Promise<unknown>> = [
    ...
    () => api.pronunciations.list('book-1'),
    () => api.pronunciations.save({ term: 'tokyo', replacement: 'Tô-ki-ô' }),
    () => api.pronunciations.remove('p1'),
    () => api.queue.enqueueSegments({ segmentIds: ['seg-1'] }),
    () => api.queue.enqueueChapter({ chapterId: 'ch-1' }),
    () => api.queue.enqueueBook('book-1'),
    () => api.queue.estimateChapter('ch-1'),
    () => api.queue.estimateBook('book-1'),
    () => api.queue.getStatus(),
    ...
    () => api.queue.cancelAll(),
    () => api.storage.getUsage(),
    () => api.storage.getChapterUsage('book-1'),
    () => api.storage.deleteChapterAudio('ch-1'),
